...
#define SCENARIO9ITERATIONTIME 10
#define SCENARIO9SHIPAMOUNT 50
//#define SCENARIO9SHIPAMOUNT 10
#define EVOLUTIONSCENARIO true
#define SCENARIO9STARTINGSHIPS 0
#define COSTRATIO 1.0
#define FIGHTERPROBABILITY 0.7

#define SCENARIO9SPEAK false

class scenario9:public scenario
{
	char Chars[100];
	int x, y, fps, tmptime;
	string result;
...
public:
	...
	metagrid(2.0),
	displaygrid(1),
	mai1(point(10.0, 0.0, 0.0), 9, 2, 1, 0, &metagrid),
	mai2(point(-10.0, 0.0, 0.0), 9, 1, 2, 0, &metagrid)
	{
	...
	void nexttime()
	{
		if(SCENARIO9SPEAK)cout<<"nexttime"<<endl<<flush;
		if(SCENARIO9SPEAK)cout<<"x: "<<x<<endl<<flush;
		x++;
		if(x%20==0)
		//if(x%10==0)
		{
		...
		if (tmptime!=time(NULL))
		{
			...
			{
				if(SCENARIO9SPEAK)cout<<"decidewhattogenerate"<<endl<<flush;
				/*mai1.newevolvedpilot();
				mai2.newevolvedpilot();*/
				mai1.decidewhattogenerate(COSTRATIO, true, true);
			...
			if(EVOLUTIONSCENARIO)
				if(y%10==0)
				{
					if(SCENARIO9SPEAK)cout<<"killoldestpilot"<<endl<<flush;
		...
		//allpilots.nexttime();
		if(SCENARIO9SPEAK)cout<<"mai nexttime"<<endl<<flush;
		mai1.nexttime();
		mai2.nexttime();
		if(SCENARIO9SPEAK)cout<<"~mai nexttime"<<endl<<flush;
		//mai.stopme();
	...
	{
		myhud=HUD;
		myhud->settopleft(string("Scenario 9"));
		vector<colour> tmp;
		...
		mai1.clear();
		mai1.restart(SCENARIO9STARTINGSHIPS, SCENARIO9SHIPAMOUNT, true, true);
		mai2.clear();
		mai2.restart(SCENARIO9STARTINGSHIPS, SCENARIO9SHIPAMOUNT, true, true);
		myasteroids.clear();
		metagrid.clear();
